...
var methods = {
    ...
    createTable : function() {
        ...
        for (var i = 0; i < options.colHeaders.length; i++) {
            ...
            if (options.columns[i].type == 'hidden') // Column type hidden
                tr += '<td id="col-' + i + '" style="display:none;">' + options.colHeaders[i] + '</td>';
            else // Other column types
                tr += '<td id="col-' + i + '" width="' + width + '" align="' + align +'"><input class="hIpt" type="text" value="' + header + '"' + (!options.allowDeleteColumn[i] ? ' disabled' : null) + '><span class="icon fa"></span></td>';
        }
                        ...
                        if (o) {
                            ...
                            if (typeof(options.contextMenu) == 'function') {
                                ...
                                if ($(e.target).parent().parent().is('thead')) {
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteColumn[o[1]] == true) {
                                    ...
                                    if (options.about) {
                                        contextMenuContent += "<a onclick=\"alert('" + options.about + "')\">About<span></span></a><hr/>";
                                    }

                                    contextMenuContent += "<a data-toggle='modal' data-target='#f_" + o[1] + "'><span class='fa fa-filter'></span>Filter Column </a>";
    ...
    setData : function(data, ignoreSpare) {
        ...
        for (j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
            ...
            // Index column
            $(tr).append('<td id="row-' + j + '" class="jexcel_label">' + parseInt(r + 1) + '</td>');
            // Data columns
    ...
    insertColumn : function (numColumns, properties, position, ignoreEvents) {
        ...
        if (options.allowInsertColumn == true) {
            ...
            for (var i = num; i < (num + numColumns); i++) {
                ...
                options.allowDeleteColumn[i] = properties.allowDeleteColumn;
    ...
    deleteColumn : function(columnNumber, numberOfColumns) {
        ...
        // Global Configuration
        if (options.allowDeleteColumn[columnNumber] == true) {
            ...
            if (options.data[0].length > 1) {
                if (parseInt(columnNumber) > -1) {
                    ...
                    options.allowDeleteColumn.splice(parseInt(columnNumber), numberOfColumns);
    ...
    getHeader : function (column) {
        var col = $(this).find('thead #col-' + column + ' input');
        if (col.length)
            return $(col).val();
    },
